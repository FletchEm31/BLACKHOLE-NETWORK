...
-- ─────────────────────────────────────────────────────────────────────────────
-- 6) weather_bronze_visual_crossing_actuals
--    Visual Crossing historical actuals — separate from NWS actuals so both
--    sources can coexist per (station, date) without unique-constraint conflicts.
--    UNIQUE per station + date. Source identified by table name.
--    Added: 2026-06-25 via migration 2026-06-25-weather-vc-actuals-table.sql
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 7) weather_bronze_era5_kmia
--    ERA5 reanalysis, KMIA bounding box (lat 25.5-26 / lon -80.5 to -80).
--    Dataset: reanalysis-era5-single-levels, 13 variables, 2025-2026.
--    One row per (valid_time, latitude, longitude). 12 time steps/day.
--    Ingested via scripts/weather/weather_era5_kmia_ingest.py.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 8) weather_bronze_era5_klax
--    ERA5 reanalysis, KLAX bounding box (lat 33.82-34.07 / lon -118.53 to -118.25).
--    Dataset: reanalysis-era5-single-levels, 9 variables, 2025-2026.
--    One row per (valid_time, latitude, longitude). 12 time steps/day.
--    No marine variables (mwd/mwp/sst/swh) -- inland bounding box, unlike KMIA.
--    Ingested via scripts/weather/weather_era5_klax_ingest.py.
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS weather_bronze_era5_klax (
    id              BIGSERIAL PRIMARY KEY,
    valid_time      TIMESTAMPTZ NOT NULL,
    latitude        NUMERIC(7,4) NOT NULL,
    longitude       NUMERIC(7,4) NOT NULL,
    ...
    CONSTRAINT weather_bronze_era5_klax_uq UNIQUE (valid_time, latitude, longitude)
);

CREATE INDEX IF NOT EXISTS idx_era5_klax_valid_time
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'bhn_trader') THEN
        ...
        GRANT SELECT, INSERT, UPDATE ON weather_bronze_era5_klax TO bhn_trader;
        GRANT USAGE, SELECT ON SEQUENCE weather_bronze_era5_klax_id_seq TO bhn_trader;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'ehuser') THEN
        ...
        GRANT SELECT, INSERT, UPDATE ON weather_bronze_era5_klax TO ehuser;
