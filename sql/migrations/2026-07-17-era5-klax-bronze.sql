-- Migration: add weather_bronze_era5_klax
-- 2026-07-17
--
-- ERA5 reanalysis bronze table for KLAX (Los Angeles International).
-- Dataset: reanalysis-era5-single-levels, 9 atmospheric variables, 2025-2026.
-- Bounding box: lat 33.82-34.07 / lon -118.53 to -118.25.
-- One row per (valid_time, latitude, longitude).
-- Data format: GRIB, downloaded via cdsapi once CDS job completes.
-- Ingestion: scripts/weather/weather_era5_klax_ingest.py --file <grib>
--
-- No marine variables (mwd/mwp/sst/swh) -- KLAX's bounding box is inland, unlike
-- KMIA's coastal box, so wave/sea-surface fields do not apply.
--
-- Run on LA (<BHN_WG_LA_IP>):
--   sudo -u postgres psql -d eventhorizon -f sql/migrations/2026-07-17-era5-klax-bronze.sql
