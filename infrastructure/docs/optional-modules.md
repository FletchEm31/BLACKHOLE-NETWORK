...
| **LibreSpeed** ✅ scheduled (2026-05-11) | Self-hosted speedtest endpoint. Useful per-region — operator can speed-test from each BHN exit to verify routing. Results to PG `speedtest_results`; HORIZON monitors latency trends. | [github.com/librespeed/speedtest](https://github.com/librespeed/speedtest) | LGPL-3.0 | **Per-node baseline** (LA, FRA, NJ, future nodes) |
...
| **Wallos** ✅ scheduled (2026-05-11) | Subscription tracker — recurring payments, renewal reminders, total monthly burn. Tracks the BHN cost stack itself (Vultr × N, Anthropic, ElevenLabs, Twilio, etc.). HORIZON reads from it via PG for morning-briefing cost surfacing. | [github.com/ellite/Wallos](https://github.com/ellite/Wallos) | AGPL-3.0 | **LA hub** — light, VPN-only |
...
| **SearXNG** ✅ scheduled (2026-05-11) | Meta-search engine — aggregates results from many engines while preserving privacy. More comprehensive than Whoogle. Pairs with Frankfurt's Tor relay for unlinkable search. | [github.com/searxng/searxng](https://github.com/searxng/searxng) | AGPL-3.0 | **Frankfurt** (privacy node), VPN-only access |
| **Tor** (non-exit middle relay) ✅ scheduled (2026-05-11) | Privacy-routing layer. Non-exit middle relay (NOT exit, NOT bridge — keeps legal exposure minimal, Vultr ToS compatible). SearXNG can route upstream via this Tor circuit for unlinkable search; future use as obfuscation layer for HORIZON-initiated outbound queries possible. **Two BHN relays planned:** Frankfurt (1 MB/s, 1500 GB/mo) + NJ (512 KB/s, 750 GB/mo — halved for trading-node coexistence). MyFamily declaration links them so Tor consensus never routes a circuit through both. | [gitlab.torproject.org/tpo/core/tor](https://gitlab.torproject.org/tpo/core/tor) | BSD-3-Clause | **Frankfurt** (privacy node) + **NJ** (trading node, secondary) |
