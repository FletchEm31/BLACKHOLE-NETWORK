...
-- Post-legacy-exclusion, this card currently shows near-zero settled
-- data — the only remaining BET_NO rows are freshly generated by the
-- live pipeline (post the write_to_ledger() dry_run fix, also
-- 2026-07-02) and haven't settled yet. That's the honest, expected state
-- of a genuinely clean track record starting from scratch, not a bug.
