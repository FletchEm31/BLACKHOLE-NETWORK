#!/usr/bin/env python3
"""
ERA5 KLAX ingestion script.
Reads a downloaded GRIB or ZIP file, parses with cfgrib/xarray,
and upserts into weather_bronze_era5_klax.

Usage:
    python3 weather_era5_klax_ingest.py --file <path> [--dry-run]
...
LAT_MIN, LAT_MAX = 33.7, 34.2
LON_MIN, LON_MAX = -118.7, -118.1


def parse_args():
    p = argparse.ArgumentParser(description="ERA5 KLAX bronze ingest")
...
def extract_grib(path: Path) -> Path:
    ...
    if path.suffix.lower() == ".zip":
        tmp = tempfile.mkdtemp(prefix="era5_klax_")
...
UPSERT_SQL = """
INSERT INTO weather_bronze_era5_klax
...
def live_run(rows: list):
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("SELECT COUNT(*), MIN(valid_time), MAX(valid_time) FROM weather_bronze_era5_klax;")
