#!/bin/bash
# ╔══════════════════════════════════════════════════════════╗
# ║      BLACKHOLE NETWORK (BHN) — PURGE / ARCHIVE SCRIPT    ║
# ║   Archives /mnt/eh-nvme-hot → /mnt/eh-hdd-cold,          ║
# ║   pg_dumps eventhorizon, VACUUMs PostgreSQL,             ║
# ║   logs every run to the purge_log table.                 ║
# ╚══════════════════════════════════════════════════════════╝
#
# NOTE: LA-deployed paths (/mnt/eh-nvme-hot, /mnt/eh-hdd-cold, the
# `eh-purge` binary at /usr/local/sbin, /etc/cron.d/eh-purge) keep
# their eh- prefixes until coordinated LA migration session. PG
# database name `eventhorizon` is also unchanged per immutable list.
#
# Usage:
...
#
# Author convention: matches bhn-node-bootstrap.sh style.
